...
from a2c_scripted_bot_example import MyScriptedBot
...
from a2c_scripted_bot_example import testowy_script
...
def main():
    ...
    env = BotBowlEnv()
    sc_agent = MyScriptedBot("scripted")
    ...
    while all_steps < num_steps:
        for step in range(steps_per_update):
            ...
            print(env.game.state.available_actions)
            sc_agent.new_game(env.game, home_team)
            print(env.game.state.available_actions)
            start_action = Action(ActionType.START_GAME)
            env.game.step(start_action)
            print("TEST #3", env.game.get_procedure())
            sc_action = sc_agent.act(env.game)
            
            print("TEST #2", sc_action)
        ...
        if all_updates % log_interval == 0 and len(episode_rewards) >= num_processes:
            ...
            # if gut:
            #     print(lista)
            if win_rate > 0.75 and difficulty > 0.95:
                gut = True
            '''
            End added
            '''
